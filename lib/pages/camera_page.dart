  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      final resultPath = await Navigator.of(context).push<String?>(
        MaterialPageRoute(builder: (c) => AnalysisPage(imagePath: saved.path)),
      );
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _tipRow(
                  'Scan only items you intend to analyze or dispose — devices, batteries, components.',
                ),
                _tipRow(
                  'Place the item on a plain, uncluttered background (solid color works best).',
                ),
                _tipRow(
                  'Make sure labels, ports, serial numbers, and maker logos are visible when possible.',
                ),
                _tipRow(
                  'Capture clear photos of intricate areas (ports, battery compartment, circuit boards) if safe.',
                ),
                _tipRow(
                  'Fill most of the frame with the device — avoid tiny distant shots.',
                ),
                _tipRow(
                  'Use natural, diffuse light. Avoid harsh backlight or strong reflections/glare.',
                ),
                _tipRow(
                  'Hold camera steady or brace against a surface; try multiple angles for better identification.',
                ),
                _tipRow(
                  'If the device is dirty, gently wipe visible dust — not liquids or solvents.',
                ),
                _tipRow(
                  'Do NOT open sealed batteries or damaged cells. Keep a safe distance from swollen batteries.',
                ),
                _tipRow(
                  'Avoid scanning wet/damaged items or anything emitting smoke or heat — follow safety procedures.',
                ),
                const SizedBox(height: 8),
                const Text(
                  'Why these help',
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              text,
              style: const TextStyle(color: Colors.black87, height: 1.28),
            ),
          ),
        ],
      ),
...
class OpenAIImageAnalyzer {
  ...
  static Future<Map<String, dynamic>> analyzeImageOnePrompt(
    String imagePath,
  ) async {
    ...
    if (apiKey.isEmpty) {
      return {
        'unknown': true,
        'message': 'OPENAI_API_KEY not set in .env — analysis skipped.',
      };
    }

    final file = File(imagePath);
    if (!await file.exists())
      return {'unknown': true, 'message': 'Image file not found'};
...
''';
    ...
    const userPrompt =
        'Identify brand, model, year, condition, components, hazards, weight (kg), material streams and disposal path. Return only a single JSON object.';
    ...
    try {
      ...
      if (resp.statusCode != 200) {
        ...
        return {
          'unknown': true,
          'message': 'OpenAI vision error ${resp.statusCode}',
          'raw': resp.body,
        };
      }
      ...
      try {
        final List<dynamic>? output = (body['output'] is List)
            ? List<dynamic>.from(body['output'])
            : null;
        if (output != null) {
          for (final outItem in output) {
            final List<dynamic>? content =
                (outItem is Map && outItem['content'] is List)
                ? List<dynamic>.from(outItem['content'])
                : null;
            if (content != null) {
        ...
        if (raw.isEmpty) {
          ...
          if (raw.isEmpty && body['answer'] != null)
            raw = body['answer'].toString();
        }
      } catch (e) {
        debugPrint(
          'Failed to extract text from Responses body: $e\nbody: ${resp.body}',
        );
      }
      ...
      try {
        ...
        return {
          'unknown': true,
          'message': 'Could not parse AI output',
          'raw': raw,
        };
      }
...
class _AnalysisPageState extends State<AnalysisPage> {
  ...
  String _normalizeCategory(String raw) {
    ...
    if (s.contains('batt') || s.contains('cell') || s.contains('battery'))
      return 'Batteries';
    if (s.contains('board') ||
        s.contains('pcb') ||
    ...
    if (s.contains('phone') ||
      ...
      return 'Devices';
    if (s.contains('accessory') ||
        s.contains('charger') ||
        s.contains('cable') ||
        s.contains('earbud') ||
        s.contains('headphone'))
      return 'Accessories';
    if (s.contains('electronic') || s.contains('electronics'))
      return 'Electronics';
    // fallback: if user or AI labelled 'e-waste' or similar, prefer Electronics
    if (s.contains('e-waste') ||
        s == 'ewaste' ||
        s == 'e waste' ||
        s == 'e waste')
      return 'Electronics';
        ...
        return [
          {
            'title': 'Safe handling',
            'body':
                'Do not puncture or crush. If terminals exposed, tape them. Store separately in non-conductive container.',
          },
          {
            'title': 'Where to take',
            'body':
                'Take to certified battery recycler or retailer take-back program (local e-waste drop-off).',
          },
          {
            'title': 'Prep steps',
            'body':
                'Place in individual plastic bags, label if damaged, avoid mixing with household trash.',
          },
        ];
      case 'Components':
        return [
          {
            'title': 'Hazard note',
            'body':
                'PCBs and capacitors may contain hazardous materials. Avoid burning or shredding at home.',
          },
          {
            'title': 'Where to take',
            'body':
                'Take to professional e-waste recycler that handles PCBs and electronic components.',
          },
          {
            'title': 'Prep steps',
            'body':
                'Keep small components in a sealed container; do not mix with general waste.',
          },
        ];
      case 'Devices':
        return [
          {
            'title': 'Data & battery',
            'body':
                'If functional, back up & wipe personal data. Remove battery if removable and recycle separately.',
          },
          {
            'title': 'Where to take',
            'body':
                'Donate working units or drop off at certified electronics recycler or manufacturer take-back.',
          },
          {
            'title': 'Prep steps',
            'body':
                'Remove SIM/memory cards, wrap fragile screens, and package to avoid leakage.',
          },
        ];
      case 'Accessories':
        return [
          {
            'title': 'Sort & separate',
            'body':
                'Separate cables, chargers, and plastics from devices; many stores accept accessory recycling.',
          },
          {
            'title': 'Where to take',
            'body':
                'Retail take-back programs or municipal e-waste collection points.',
          },
          {
            'title': 'Prep steps',
            'body':
                'Bundle cables neatly; avoid mixing with hazardous components.',
          },
        ];
      case 'Electronics':
      default:
        return [
          {
            ...
            'body':
                'Electronics can contain hazardous parts; do not incinerate or dump in regular trash.',
          },
          {
            'title': 'Where to take',
            'body':
                'Use certified e-waste recyclers or manufacturer/retailer take-back services.',
          },
          {
            'title': 'Prep steps',
            'body':
                'Remove batteries & data storage if possible; package to avoid leakage or short circuits.',
          },
        ];
    }
  }
  ...
  Future<void> _analyze() async {
    ...
    try {
      ...
      final Map<String, dynamic> result =
          await OpenAIImageAnalyzer.analyzeImageOnePrompt(
            widget.imagePath,
          ).timeout(
            const Duration(seconds: 50),
            onTimeout: () => {'unknown': true, 'message': 'Analysis timed out'},
          );
      ...
      if (result.containsKey('unknown') && result['unknown'] == true) {
        setState(() {
          ...
          _descriptionController.text =
              (result['message'] ?? 'unknown ewaste - please scan an ewaste')
                  .toString();
      ...
      try {
        ...
        _descriptionController.text = rawDesc.length > 400
            ? '${rawDesc.substring(0, 380)}…'
            : rawDesc;
        ...
        if (comp is List) {
          _components = comp.map((e) => e.toString()).toList();
        } else if (comp is String && comp.isNotEmpty)
          _components = comp.split(',').map((e) => e.trim()).toList();

        final hz = result['hazards'];
        if (hz is List) {
          _hazards = hz.map((e) => e.toString()).toList();
        } else if (hz is String && hz.isNotEmpty)
          _hazards = hz.split(',').map((e) => e.trim()).toList();
        ...
        final rawDisposal =
            (result['disposalPath'] ??
                    result['disposal_path'] ??
                    result['disposal'] ??
                    '')
                .toString();
        _disposalPath = rawDisposal.length > 600
            ? '${rawDisposal.substring(0, 580)}…'
            : rawDisposal;

        _rawMetrics = (result['metrics'] is Map)
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: _infoCard(
                            ...
                            label: 'Brand',
                            value: _brand.isEmpty ? 'Unknown' : _brand,
                            color: primary,
                          ),
                        ),
                        const SizedBox(width: 10),
                        Expanded(
                          child: _infoCard(
                            ...
                            label: 'Model',
                            value: _model.isEmpty ? 'Unknown' : _model,
                            color: primary,
                          ),
                        ),
                      ],
                    ...
                    _materialStreams.isEmpty
                        ...
                        : Column(
                            children: _materialStreams.entries
                                .map((e) => _materialRow(e.key, e.value))
                                .toList(),
                          ),
                              ...
                              child: Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(height: 10),
                                    Text(
                                      'Search for service & repair centers for $_brand near you.',
                                      style: const TextStyle(
                                        color: Colors.black87,
                                      ),
                                    ),
                            ...
                            child: Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 10),
                                  const Text(
                                    'Find certified e-waste drop-off points and recycler services near you.',
                                    style: TextStyle(color: Colors.black87),
                                  ),
                                  const SizedBox(height: 10),
                                  Align(
